...
const isFormUrlEncoded = require('./check/isFormUrlEncoded');
...
class Client {
  ...
  setup(agree) {
    ...
    agree = completion(agree);
    const options = extract.outgoingRequest(agree.request, this);
    const hasFormUrlEncoded = isFormUrlEncoded(agree.request);
    const hasContentJSON = isContentJSON(agree.request) || typeof agree.request.body === 'object';
    const content = 
      hasFormUrlEncoded ? url.format(format(agree.request.body, agree.request.values)) :
      hasContentJSON ? JSON.stringify(format(agree.request.body, agree.request.values)) :
      agree.request.body;
